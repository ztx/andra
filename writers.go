...
func fieldAssignmentPayloadToModel(model *NoSqlModelDefinition, ut *design.UserTypeDefinition, verpkg, v, mtype, utype string) string {
...
func fieldAssignmentModelToType(model *NoSqlModelDefinition, ut *design.ViewDefinition, v, mtype, utype string) string {
	tmp := 1
	var fieldAssignments []string
...
func fieldAssignmentTypeToModel(model *NoSqlModelDefinition, ut *design.UserTypeDefinition, utype, mtype string) string {
	// Get a sortable slice of field names
	var keys []string
	for k := range model.NoSqlFields {
		keys = append(keys, k)
...
func viewSelect(ut *NoSqlModelDefinition, v *design.ViewDefinition) string {
	obj := v.Type.(design.Object)
	var fields []string
	for name := range obj {
		if obj[name].Type.IsPrimitive() {
			if strings.TrimSpace(name) != "" && name != "links" {
				bf, ok := ut.NoSqlFields[codegen.Goify(name, true)]
				if ok {
...
func viewFieldNames(ut *NoSqlModelDefinition, v *design.ViewDefinition) []string {
	obj := v.Type.(design.Object)
	var fields []string
	for name := range obj {
		if obj[name].Type.IsPrimitive() {
...
type mediaTemplate struct {
	...
	ViewName string
	Model    *NoSqlModelDefinition
	View     *design.ViewDefinition
}

// {{ template "Media" (newMediaTemplate $rmt $vname $ut $vp $vpn)}}
func newMediaTemplate(mtd *design.MediaTypeDefinition, vn string, view *design.ViewDefinition, model *NoSqlModelDefinition) *mediaTemplate {
